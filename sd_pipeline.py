...
class Decoding_nonbatch_SDPipeline(StableDiffusionPipeline):
    ...
    def score_to_probs(self, scores, alpha=10.0):
        """
        Convert raw reward scores into a probability vector.

        scores: numpy array of shape [duplicate]
        alpha: temperature. Smaller alpha = greedier; larger alpha = more uniform.
        """
            ...
            for i, t in enumerate(timesteps):
                ...
                if i < len(timesteps) - 1:
                    ...
                    for b in range(self.batch_size):
                        start = b * self.duplicate
                        end = (b + 1) * self.duplicate

                        scores_b = weights[start:end]
                        probs_b = self.score_to_probs(scores_b, alpha=self.alpha)

                        chosen_dup = np.random.choice(self.duplicate, p=probs_b)
                        index_chosen.append(start + chosen_dup)
